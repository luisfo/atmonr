...
# load 'gstream.collection' from Robj or use load_graphStreams.R
load("gstream.collection-test-lst.Robj") # loads 'gstream.collection'

a_data <- list(dataset = "thiers.12", metric = c("diameter", "nofComponents", "gcRatio", "deg.dist.pow"), 
               topo="dissimilarity")
...
for (metric in a_data$metric) {
  ...
  #apply AdaM with advanced bandit balancing function on metric and topology stream
